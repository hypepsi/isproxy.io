...
import { useRouter } from 'next/navigation'; // 导入 useRouter 用于页面跳转
...
export default function LoginPage() {
  ...
  const [successMessage, setSuccessMessage] = useState(''); // 保留成功消息状态，用于显示 "登录成功！正在跳转..."
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // 登录成功
      setSuccessMessage(data.message || '登录成功！正在跳转...'); // 显示成功消息
      ...
      if (token) {
        ...
        // 可以在这里存储用户信息到 localStorage，如果需要的话
        // localStorage.setItem('userData', JSON.stringify(data.user));

        // 3. 延迟一小段时间后跳转到用户仪表盘，以便用户看到成功消息
        setTimeout(() => {
          router.push('/user-dashboard'); // 跳转到用户仪表盘
        }, 1000); // 1秒后跳转
      } else {
        // 如果API成功响应但没有token，这是一个潜在问题
        console.error('登录成功但未收到Token');
        setError('登录凭证获取失败，请稍后再试。');
      }
      ...
      <main className="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl py-10 lg:py-16 flex items-center justify-center">
        <div className="card w-full max-w-md bg-base-100 shadow-xl border border-base-300/40 rounded-lg">
          <div className="card-body p-6 sm:p-8">
            <h1 className="text-2xl font-bold text-center mb-6 text-base-content">登录您的账户</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end text-xs mt-1">
                <Link href="/forgot-password" className="link link-hover text-base-content/70 hover:text-primary">
                  忘记密码?
                </Link>
              </div>
            ...
            <div className="text-center mt-6">
              <p className="text-sm text-base-content/70">
                还没有账户?
                <Link href="/register" className="link link-primary hover:link-secondary ml-1 font-medium">
                  立即注册
                </Link>
              </p>
            </div>
